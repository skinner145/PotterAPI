 ...
 * @Last modified time: 2019-11-07T20:20:15+00:00
 */
...
updateSearch(ev){
 ...
 render(){
   ...
   let filteredMembers = characters.filter(
     (member) => {
   ...
   const renderItems = currentItems.map((character) =>{
     return <ListGroup.Item  onClick={this.memberToggle} style={{cursor:"pointer"}} data-key={character._id}>{character.name}</ListGroup.Item>
   });
